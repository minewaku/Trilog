...
public class FileUploadUtil {
    ...
    public static void assertImageAllowed(MultipartFile file, String pattern) {
        ...
        if(size > MAX_IMAGE_SIZE) {
            throw new RuntimeException(MessageUtil.MESSAGES.getString("cloudinary.invalid.media.size.image") + MAX_IMAGE_SIZE / 1024 /1024 + "MB");
        }

        final String fileName = file.getOriginalFilename();
        if (!isAllowedExtension(fileName, pattern)) {
            throw new RuntimeException(MessageUtil.MESSAGES.getString("cloudinary.invalid.media.type.image"));
        }
    }
    ...
    public static void assertVideoAllowed(MultipartFile file, String pattern) {
        final long size = file.getSize();
        if(size > MAX_IMAGE_SIZE) {
            throw new RuntimeException(MessageUtil.MESSAGES.getString("cloudinary.invalid.media.size.video") + MAX_VIDEO_SIZE / 1024 / 1024 + "MB");
        }

        final String fileName = file.getOriginalFilename();
        if (!isAllowedExtension(fileName, pattern)) {
            throw new RuntimeException(MessageUtil.MESSAGES.getString("cloudinary.invalid.media.type.video"));
        }
    }
