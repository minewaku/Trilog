...
public class MediaService implements IMediaService {
    ...
    public MediaDTO findById(Integer id) {
        try {
            Media file = imageRepository.findById(id).orElseThrow(() -> new RuntimeException(MessageUtil.MESSAGES.getString("error.get.media")));
    ...
    public MediaDTO create(MediaDTO file) {
        try {
            if(file.getId() == null || imageRepository.findById(file.getId()).isEmpty()) {         
                ...
                throw new RuntimeException(MessageUtil.MESSAGES.getString("error.create.media"));
            }
        } catch(Exception e) {
            throw new RuntimeException(e.getMessage(), e);
        }
    ...
    public MediaDTO update(int id, MediaDTO file) {
        try {
            imageRepository.findById(id).orElseThrow(() -> new RuntimeException(MessageUtil.MESSAGES.getString("error.get.media")));
            Media savedFile = imageRepository.save(mapper.dtoToEntity(file));
            return mapper.entityToDto(savedFile);
        } catch(Exception e) {
            throw new RuntimeException(e.getMessage(), e);
